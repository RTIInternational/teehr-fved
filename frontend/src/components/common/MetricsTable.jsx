...
const MetricsTable = ({
  ...
  const {
    ...
    if (!tableProperties?.group_by || !tableProperties?.metrics || !Array.isArray(metrics)) {
      ...
      if (Array.isArray(metrics) && metrics.length > 0) {
        ...
        return {
          headers: ['Field', 'Value'],
          rows: entries.map(([key, value]) => [key, formatCellValue(key, value)]),
          hasLeadTimeBin: false,
        };
      }
  ...
  const plotData = useMemo(() => {
    ...
    const groupByFields = tableProperties.group_by;
    const metricFields = tableProperties.metrics;
    const leadTimeBinField = groupByFields.find(
      (field) =>
    ...
    const nonLeadTimeGroupByFields = groupByFields.filter(
      (field) =>
        !field.toLowerCase().includes('lead_time_bin') &&
        !field.toLowerCase().includes('forecast_lead_time_bin')
    );
    ...
    filteredData.forEach((item) => {
      ...
      const groupKey =
        relevantGroupByFields.length > 0
    ...
    Object.entries(groupedData).forEach(([groupKey, groupData]) => {
      ...
      metricFields
        ...
        .forEach((metric) => {
          ...
          const traceName = relevantGroupByFields.length > 0 ? `${metric} (${groupKey})` : metric;
  ...
  return (
    <div className="metrics-table-container">
        ...
        <div className="metrics-table-header">
          ...
          <div className="d-flex align-items-center gap-2">
            <span className="metrics-count">
              ({processedRows.length} rows{hasActiveFilters ? ` of ${rawRows.length}` : ''})
            </span>
          </div>
        </div>
      )}
      ...
      {!showTitle && hasLeadTimeBin && !onViewModeChange && (
        <div
          className="metrics-toggle-header"
          style={{
            padding: '8px 12px',
        ...
        >
          ...
          <span className="metrics-count small text-muted">
            ({processedRows.length} rows{hasActiveFilters ? ` of ${rawRows.length}` : ''})
          </span>
        </div>
      )}
      ...
      {currentViewMode === 'filters' && tableProperties?.group_by?.length > 0 ? (
                ...
                <div key={index} className="col-lg-4 col-md-6 mb-2">
                  ...
                  <MultiSelectDropdown
                    ...
                    onChange={(newSelected) =>
                      setFilters((prev) => ({ ...prev, [index]: newSelected }))
                    }
                    allSelectedText="All"
                ...
                <div className="alert alert-info py-2 d-flex justify-content-between align-items-center mb-0">
                  <span>
                    ...
                    {Object.values(filters).reduce(
                      (total, f) => total + (Array.isArray(f) ? f.length : 0),
                      0
                    )}{' '}
                    selections
                  </span>
        ...
        <div
          key="plot-view"
          className="metrics-plot-container"
          style={{ height: '100%', minHeight: 0, padding: '10px', display: 'flex' }}
        >
        ...
        >
          ...
          <SharedDataTable
            ...
            getCellProps={(_row, _header, _rowIndex, cellIndex) => ({
              className:
                cellIndex < (tableProperties?.group_by?.length || 0)
                  ? 'group-by-cell'
                  : 'metric-cell',
            })}
          />
